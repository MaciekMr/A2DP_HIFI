...
void BluetoothA2DPSink::app_task_handler(void *arg)
{
    ...
    while (true) {
        if (!app_task_queue){
            ...
            delay(100);
        } else if (pdTRUE == xQueueReceive(app_task_queue, &msg, (TickType_t)portMAX_DELAY)) {
...
void BluetoothA2DPSink::app_gap_callback(esp_bt_gap_cb_event_t event, esp_bt_gap_cb_param_t *param)
{
    switch (event) {
        ...
        case ESP_BT_GAP_CFM_REQ_EVT: {
                ...
                ESP_LOGI(BT_AV_TAG, "ESP_BT_GAP_CFM_REQ_EVT Please confirm the passkey: %d", (int)param->cfm_req.num_val);
...
void BluetoothA2DPSink::handle_audio_cfg(uint16_t event, void *p_param) {
    ...
    ESP_LOGI(BT_AV_TAG, "a2dp audio_cfg_cb , sample_rate %d", (int)i2s_common.get_sample_rate() );
    if (sample_rate_callback!=nullptr){
        sample_rate_callback(i2s_common.get_sample_rate());
    }

    // for now only SBC stream is supported
    if (player_init == false && i2s_common.get_i2s_output() && a2d->audio_cfg.mcc.type == ESP_A2D_MCT_SBC) {
        ...
        {
            ESP_LOGE(BT_AV_TAG, "i2s_set_clk failed with samplerate=%d", (int)i2s_common.get_sample_rate());
        }

        if(i2s_common.set_sample_channels()!=ESP_OK){
            ESP_LOGE(BT_AV_TAG, "i2s_set_clk failed with samplerate=%d", (int)i2s_common.get_sample_rate());
        } else {
            ESP_LOGI(BT_AV_TAG, "audio player configured, samplerate=%d", (int) i2s_common.get_sample_rate());
...
uint16_t BluetoothA2DPSink::sample_rate(){
    return i2s_common.get_sample_rate(); 
}
...
void BluetoothA2DPSink::av_hdl_avrc_evt(uint16_t event, void *p_param)
{
    ...
    case ESP_AVRC_CT_REMOTE_FEATURES_EVT: {
        ESP_LOGI(BT_AV_TAG, "AVRC remote features %x", (unsigned int)rc->rmt_feats.feat_mask);
        break;
    }
    ...
    case ESP_AVRC_CT_GET_RN_CAPABILITIES_RSP_EVT: {
        ESP_LOGI(BT_AV_TAG, "remote rn_cap: count %d, bitmask 0x%x", rc->get_rn_caps_rsp.cap_count,
                 (unsigned int)rc->get_rn_caps_rsp.evt_set.bits);
...
size_t BluetoothA2DPSink::i2s_write_data(const uint8_t* data, size_t item_size){
    ...
    if (this->i2s_common.get_config().bits_per_sample == I2S_BITS_PER_SAMPLE_16BIT){
        ...
        if (this->i2s_common.get_config().bits_per_sample > 16){
            ...
            ESP_LOGE(BT_AV_TAG, "invalid bits_per_sample: %d", this->i2s_common.get_bits_per_sample());    
        }
    }
    return item_size;
}
...
void BluetoothA2DPSink::av_hdl_avrc_tg_evt(uint16_t event, void *p_param)
{
    ...
    case ESP_AVRC_TG_REGISTER_NOTIFICATION_EVT: {
        ESP_LOGI(BT_AV_TAG, "AVRC register event notification: %d, param: 0x%x", rc->reg_ntf.event_id, (unsigned int)rc->reg_ntf.event_parameter);
    ...
    case ESP_AVRC_TG_REMOTE_FEATURES_EVT: {
        ESP_LOGI(BT_AV_TAG, "AVRC remote features %x, CT features %x", (unsigned int) rc->rmt_feats.feat_mask, (unsigned int)rc->rmt_feats.ct_feat_flag);
        break;
    }
